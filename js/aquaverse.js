...
(function() {
  let w = angular.module(
    ...
    ["locationApp", function(locationApp) {},]
  );

  w.controller("aquaverseCtrl", [
    "$scope",
    "$http",
    "$sce",
    "$mdMedia",
    ...
    function($scope, $http, $sce, $mdMedia, $mdSidenav, $compile, $location) {
        ...
        if (
          ...
          if (content.type != "external-link") {
            ...
            if (push) {
              history.pushState(
                {
                  section: $scope.state.section,
                  active_content: $scope.state.active_content
                },
                "State",
                ""
              );
          ...
          switch (content.type) {
            case "local-md":
              $http.get(content.path).then(response => {
                $("#main-md")
                  .empty()
                  .html(conv.makeHtml(response.data));
                if (config.nextprev) {
                  $("#main-md").append(
                    $compile(`<div ng-include="'prevnext.html'"></div>`)($scope)
                  );
                }
                if (config.copyright) {
                  $("#main-md").append(
              ...
              break;
            case "local-webpage":
              $scope.state.iframe_url = $sce.trustAsResourceUrl(content.path);
              break;
            case "local-html":
              break;
            case "external-link":
      ...
      function nav_indices() {
        if ($scope.state.section && $scope.state.active_content) {
          let i = section_index($scope.state.section.category),
            j = content_index(
              $scope.navigation[i].contents,
              $scope.state.active_content.name
            );
          return [i, j];
        } else {
          return [0, 0];
      ...
      window.onpopstate = function(event) {
        if (event.state) {
          $scope.easy_select(
            event.state.section.category,
            event.state.active_content.name,
            false
          );
      ...
      if (config.get_releases) {
        $http
          .get("https://api.github.com/repos/klavinslab/aquarium/releases")
          .then(response => {
            ...
            for (var n in $scope.releases) {
              $scope.releases[n].md = $sce.trustAsHtml(
                conv.makeHtml($scope.releases[n].body)
              );
              highlight_code();
            }
          })
          .catch(error =>
            console.log(
              "Github did not return release array. Probably too many requests."
            )
          );
      }
      if (config.workflows) {
        get_workflow_configs($http).then(
          configs => ($scope.workflows = configs)
        );
      }
    }
  ]);
